...
public class JSONControl {
        ...
        try {
            ...
            for (File f : files) {
                ...
                try {
                    ...
                    // Wrong keystore / wrong password / try next file
                    continue;
                }
            }
    ...
    try {
        String path = "MainAccounts/" + this.user.getMainFileNum() + ".enc";
        byte[] encrypted = Files.readAllBytes(Paths.get(path));
        byte[] plain = cipherTool.decrypt(encrypted);
        this.user = myMapper.readValue(convertToString(plain), User.class);
    } catch (Exception e) {
        System.out.println("Error reloading user: " + e.getMessage());
    }
}
    ...
    public void loadAccountFiles() {
      try {
          ...
          for (User.SubUserMeta su : user.getSubAccounts()) {
              try {
    ...
    public void addAccount(String aUrl, String aUsername, String aPassword, ArrayList tags) {
        ...
        Account newAccount = new Account(aUrl, aUsername, aPassword, tags, createdDate, lastUsed, this.user.getAccountFileNum());
    ...
    public void storeAccountData(Account anAccount) {
        try {
            ...
            if (this.cipherTool == null) {
                this.cipherTool = new Security(this.user.getUsername(), this.user.getPassword(), this.user.getMainFileNum());
            }

            String jsonAccount = myMapper.writeValueAsString(anAccount);
            byte[] encrypted = cipherTool.encrypt(convertToBytes(jsonAccount));
            Path path = Paths.get(folderPath, this.user.getAccountFileNum() + ".enc");
            Files.write(path, encrypted);

            int fileNumber = Integer.parseInt(this.user.getAccountFileNum());
            fileNumber += 1;
            this.user.setAccountFileNum(String.valueOf(fileNumber));

            storeMainAccount(this.user.getUsername(), this.user.getPassword());
    ...
    public void storeMainAccount(String username, String password) {
        try {
            ...
            if (this.cipherTool == null) {
                this.cipherTool = new Security(username, password, this.user.getMainFileNum());
            }

            String jsonUser = myMapper.writeValueAsString(this.user);
            byte[] encrypted = cipherTool.encrypt(convertToBytes(jsonUser));
            Path path = Paths.get("MainAccounts", this.user.getMainFileNum() + ".enc");
            Files.write(path, encrypted);
    ...
    public void storeSubAccount(String name, String username, String password) {
        try {
            ...
            subUser.setIsSub(true);
            subUser.setParentUid(this.user.getuid());
            ...
            this.user.getSubAccounts().add(meta);
    ...
    public void deleteSubUser(User.SubUserMeta target) {
        try {
            ...
            this.user.getSubAccounts().removeIf(su -> su.uid.equals(target.uid));
            storeMainAccount(this.user.getUsername(), this.user.getPassword());
    ...
    public void authorizeDevice(String deviceId) {
        ...
        this.user.authorizeDevice(deviceId);
        storeMainAccount(this.user.getUsername(), this.user.getPassword());
    }
    ...
    public void removeAuthorizedDevice(String deviceId) {
        if (this.user == null) return;
        this.user.removeAuthorizedDevice(deviceId);
        storeMainAccount(this.user.getUsername(), this.user.getPassword());
    }

    // Return the list 
    public ArrayList<String> getAuthorizedDevices() {
        if (this.user == null) return new ArrayList<>();
        return this.user.getAuthorizedDevices();
    }
