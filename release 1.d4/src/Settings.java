...
public class Settings {
    ...
    public void showSettings(JSONControl controller) {
        ...
        saveButton.setOnAction(e -> System.out.println("Settings have been saved"));
        ...
        changeUsername.setOnAction(e -> {
            ...
            result.ifPresent(newName -> {
                controller.getUser().pastUsername = controller.getUser().username;
        ...
        changePassword.setOnAction(e -> {
            ...
            result.ifPresent(newPass -> {
                controller.getUser().pastPassword = controller.getUser().password;
    ...
    private void showSubUserSelectionWindow(JSONControl controller, String action) {
        ...
        for (SubUser su : controller.getUser().getsubAccounts()) {
            list.getItems().add(su.getUsername());
        }
        ...
        confirm.setOnAction(e -> {
            ...
            SubUser target = controller.getUser().getsubAccounts()
                    .stream()
                    .filter(su -> su.getUsername().equals(selected))
                    .findFirst()
                    .orElse(null);
    ...
    private void showSubUserUpdateWindow(JSONControl controller, SubUser target) {
        Stage stage = new Stage();
        stage.setTitle("Update SubUser");

        TextInputDialog userDialog = new TextInputDialog(target.getUsername());
        ...
        newUser.ifPresent(targetUsername -> target.username = targetUsername);

        TextInputDialog passDialog = new TextInputDialog(target.getPassword());
        ...
        newPass.ifPresent(targetPassword -> target.password = targetPassword);

        controller.updateSubUser(target);
