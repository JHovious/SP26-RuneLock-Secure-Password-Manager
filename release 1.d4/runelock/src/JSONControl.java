...
public class JSONControl {
    ...
    public void createMainAccount(String aUsername, String aPassword){
        ...
        /*int toolCheck = 0;
        String newUID = this.generateUID();
        User newUser = new User(aUsername, aPassword, false, newUID, newUID, "", "");
        try{
            if (this.cipherTool == null){
                this.cipherTool = new Security(aUsername, aPassword, newUID);
            }else{
            ...
            if (toolCheck == 1){//Secondary main user being created
                try{//Neeed to make main account file
                    String jsonUser = myMapper.writeValueAsString(newUser);
            ...
            }else{//First main user for runelock
                this.user = newUser;
                System.out.println("Testing 10");
                cipherTool.generateKeyStore(this.user);
                saveEncryptedUser();//Save user to encrypted json format
            }
